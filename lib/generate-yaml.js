...
function buildYaml(opts, deps = {}) {
  ...
  if (dnsAntiLeak) {
    params.dns = {
      ...
      // 而上面的规则集依赖 raw.githubusercontent.com。拉取失败时列表就是空的 ——
      ...
      "fake-ip-filter": [
        ...
        "+.msftncsi.com", "+.msftconnecttest.com", // 补回被覆盖掉的内核默认（Windows 连通性检测）
        "+.in-addr.arpa", "+.ip6.arpa",            // 反向解析
        "+.pool.ntp.org", "time.windows.com", "time.apple.com", // 校时
      ],
    };
